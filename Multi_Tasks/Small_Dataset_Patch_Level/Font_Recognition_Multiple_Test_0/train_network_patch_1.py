...
def create_save_dataset():
    ...
    train_imgs_obj = WordImageDS(training_imag_paths, 400, transform=data_transforms['train'])  # if we don't pass the
    # 3rd parameter then it takes default value which is -1; that means take all the images
    mn_dataset_loader_train = torch.utils.data.DataLoader(dataset=train_imgs_obj, batch_size=200, shuffle=True,
                                                          num_workers=6, drop_last=True)

    validation_imgs_obj = WordImageDS(validation_imag_paths, 100, transform=data_transforms['val'])  # if we don't pass
    # the 3rd parameter then it takes default value which is -1; that means take all the images
    mn_dataset_loader_validation = torch.utils.data.DataLoader(dataset=validation_imgs_obj, batch_size=200,
...
def main():
    logger = Logger('/home/tmondal/Python_Projects/Font_Recognition/Patch_Level/Backup_Data/Font_Recognition_Multiple_Test_0/logs/'+"save_training_params_crossnet_224"+'.log', True)
