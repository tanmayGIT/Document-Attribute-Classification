...
class Trainer:
    def __init__(self, model, optimizer, lr_schedule, is_use_cuda, train_data_loader,
        ...
        if train_me_where == "from_middle":
            epoch_num = "52"
            checkpoint = torch.load('/home/tmondal/Python_Projects/Font_Recognition/Patch_Level/backup/Font_Recognition_Multiple_Test_1/checkpoint/' + model_name + '/Models_epoch_' + epoch_num + '.ckpt')
    ...
    def _save_best_model(self):
        ...
        if not os.path.isdir('/home/tmondal/Python_Projects/Font_Recognition/Patch_Level/backup/Font_Recognition_Multiple_Test_1/checkpoint/' + self.model_name):
            os.makedirs('/home/tmondal/Python_Projects/Font_Recognition/Patch_Level/backup/Font_Recognition_Multiple_Test_1/checkpoint/' + self.model_name)

        torch.save(state,
                   '/home/tmondal/Python_Projects/Font_Recognition/Patch_Level/backup/Font_Recognition_Multiple_Test_1/checkpoint/' + self.model_name + '/Models' + '_epoch_%d' % self.cur_epoch + '.ckpt')  # Notice
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for inputs_1, inputs_2, single_image_label_scans, single_image_label_sizes, single_image_label_types, \
                    ...
                    if self.is_use_cuda:
                        inputs_2, single_image_label_scans, single_image_label_sizes, single_image_label_types, \
                        single_image_label_emphas = inputs_2.cuda(), single_image_label_scans.cuda(), \
                    ...
                    running_loss += loss.item() * inputs_2.size(0)
                    running_loss0 += loss0.item() * inputs_2.size(0)
                    running_loss1 += loss1.item() * inputs_2.size(0)
                    running_loss2 += loss2.item() * inputs_2.size(0)
                    running_loss3 += loss3.item() * inputs_2.size(0)
