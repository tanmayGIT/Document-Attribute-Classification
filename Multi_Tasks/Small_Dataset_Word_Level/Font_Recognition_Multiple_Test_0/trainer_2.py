...
class Trainer:
    def __init__(self, model, optimizer, lr_schedule, is_use_cuda, train_data_loader,
        ...
        if train_me_where == "from_middle":
            epoch_num = "44"
            checkpoint = torch.load('/home/tmondal/Python_Projects/Font_Recognition/Word_Level/Backup_Data/Font_Recognition_Multiple_Word_Test/checkpoint_2/' + model_name + '/Models_epoch_' + epoch_num + '.ckpt')
    ...
    def _save_best_model(self):
        ...
        if not os.path.isdir('/home/tmondal/Python_Projects/Font_Recognition/Word_Level/Backup_Data/Font_Recognition_Multiple_Word_Test/checkpoint_2/' + self.model_name):
            os.makedirs('/home/tmondal/Python_Projects/Font_Recognition/Word_Level/Backup_Data/Font_Recognition_Multiple_Word_Test/checkpoint_2/' + self.model_name)

        # if not os.path.isdir('./checkpoint_best_weights/' + self.model_name):
            # os.makedirs('./checkpoint_best_weights/' + self.model_name)

        torch.save(state,
                   '/home/tmondal/Python_Projects/Font_Recognition/Word_Level/Backup_Data/Font_Recognition_Multiple_Word_Test/checkpoint_2/' + self.model_name + '/Models' + '_epoch_%d' % self.cur_epoch + '.ckpt')  # Notice
