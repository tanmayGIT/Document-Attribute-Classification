...
We create the data loader by combining them both. Then during the training, we are using only word images for training. We want to see whether it is giving the same results as we were getting when using a separate training dataset for words only.

More important thing is that the network architecture here can't accept two input. It can only access one input (this network is different than the network which can acess two inputs). 
