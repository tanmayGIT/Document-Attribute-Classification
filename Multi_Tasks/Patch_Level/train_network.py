...
def create_save_dataset_small():
    ...
    train_imgs_obj = wordSmallDataset(training_imag_paths, 500, transform=data_transforms['train'])
    validation_imgs_obj = wordSmallDataset(validation_imag_paths, 500, transform=data_transforms['val'])
...
def create_save_dataset():
    ...
    mn_dataset_loader_train = torch.utils.data.DataLoader(dataset=train_imgs_obj,batch_size=500, shuffle=True, num_workers=0)
    ...
    mn_dataset_loader_validation = torch.utils.data.DataLoader(dataset=validation_imgs_obj,batch_size=500, shuffle=True, num_workers=0)
...
def main():
    logger = Logger('/home/tmondal/Python_Projects/Patch_Level/Font_Recognition_Multiple/logs/'+"save_training_params_crossnet_224"+'.log', True)
