...
# These all function is for 1D network
...
class FlatAvgPool1D(nn.Module):

    def __init__(self):
        super(FlatAvgPool1D, self).__init__()
...
class FeatureLinearLayer1D(nn.Module):

    def __init__(self):
        super(FeatureLinearLayer1D, self).__init__()
...
class CombineMultiOutputModelWeightedConcat(nn.Module):
    def __init__(self, model_core, dd):
        ...
        self.x2 = nn.Linear(256, 128)  # 512; no because it is not for second input. It is to apply 2nd linear layer
        nn.init.xavier_normal_(self.x2.weight)

        self.x3 = nn.Linear(128, 64)
        nn.init.xavier_normal_(self.x3.weight)

        self.x4 = nn.Linear(64, 32)
        nn.init.xavier_normal_(self.x4.weight)

        self.x5 = nn.Linear(32, 16)
        nn.init.xavier_normal_(self.x5.weight)

        self.x6 = nn.Linear(16, 16)
        nn.init.xavier_normal_(self.x6.weight)
        ...
        self.xw1 = nn.Linear(32, 16)
        nn.init.xavier_normal_(self.xw15.weight)

        self.y1 = nn.Linear(16, 2)  # this is to get 2 weight values only
    ...
    def forward(self, x1_input, x2_input):
        ...
        x1_out = self.bn1(Fun.relu(self.x1(x1_out)))
        x1_out = self.bn2(Fun.relu(self.x2(x1_out)))
        x1_out = self.bn3(Fun.relu(self.x3(x1_out)))
        x1_out = self.bn4(Fun.relu(self.x4(x1_out)))
        x1_out = self.bn5(Fun.relu(self.x5(x1_out)))

        x2_out = self.resnet_model(x2_input)
        x2_out = self.bn1(Fun.relu(self.x1(x2_out)))
        x2_out = self.bn2(Fun.relu(self.x2(x2_out)))
        x2_out = self.bn3(Fun.relu(self.x3(x2_out)))
        x2_out = self.bn4(Fun.relu(self.x4(x2_out)))
        x2_out = self.bn5(Fun.relu(self.x5(x2_out)))

        x11 = self.bn5(Fun.relu(self.x6(x1_out)))
        x12 = self.bn5(Fun.relu(self.x6(x1_out)))
        x13 = self.bn5(Fun.relu(self.x6(x1_out)))
        x14 = self.bn5(Fun.relu(self.x6(x1_out)))

        x21 = self.bn5(Fun.relu(self.x6(x2_out)))
        x22 = self.bn5(Fun.relu(self.x6(x2_out)))
        x23 = self.bn5(Fun.relu(self.x6(x2_out)))
        x24 = self.bn5(Fun.relu(self.x6(x2_out)))

        # dynamic weight
        xw11 = self.bn4(Fun.relu(self.xw1(torch.cat((x11, x21), dim=1)))) # concat 16 and 16 nodes which makes it 32 and then it becomes 32 -> 16
        xw11 = self.bn5(Fun.relu(self.y1(xw11))) # make it 16 -> 2

        xw21 = self.bn4(Fun.relu(self.xw1(torch.cat((x12, x22), dim=1))))
        xw21 = self.bn5(Fun.relu(self.y1(xw21)))

        xw31 = self.bn4(Fun.relu(self.xw1(torch.cat((x13, x23), dim=1))))
        xw32 = self.bn5(Fun.relu(self.y1(xw31)))

        xw41 = self.bn4(Fun.relu(self.xw1(torch.cat((x14, x24), dim=1))))
        xw41 = self.bn5(Fun.relu(self.xw42(xw41)))

        # heads
        y1word = Fun.softmax(self.y1o(x11), dim=1)
        y2word = Fun.softmax(self.y2o(x12), dim=1)
        y3word = Fun.softmax(self.y3o(x13), dim=1)
        y4word = Fun.softmax(self.y4o(x14), dim=1)  # should be sigmoid

        y1patch = Fun.softmax(self.y1o(x21), dim=1)
        y2patch = Fun.softmax(self.y2o(x22), dim=1)
        y3patch = Fun.softmax(self.y3o(x23), dim=1)
        y4patch = Fun.softmax(self.y4o(x24), dim=1)  # should be sigmoid
...
class CombineMultiOutputModelWeightedConcat1(nn.Module):
    def __init__(self, model_core, dd):
        super(CombineMultiOutputModelWeightedConcat1, self).__init__()
        ...
        # network for dynamic weight
        self.xw1 = nn.Linear(32, 16)
        nn.init.xavier_normal_(self.xw1.weight)
    ...
    def forward(self, x1_input, x2_input):
        ...
        # dynamic weight
        # concat 16 and 16 nodes which makes it 32 and then it becomes 32 -> 16
        xw11 = self.bn5(Fun.relu(self.xw1(torch.cat((x11, x21), dim=1))))
        xw11 = (Fun.relu(self.y1(xw11)))  # make it 16 -> 2

        xw21 = self.bn5(Fun.relu(self.xw1(torch.cat((x12, x22), dim=1))))
        xw21 = (Fun.relu(self.y1(xw21)))

        xw31 = self.bn5(Fun.relu(self.xw1(torch.cat((x13, x23), dim=1))))
        xw31 = (Fun.relu(self.y1(xw31)))

        xw41 = self.bn5(Fun.relu(self.xw1(torch.cat((x14, x24), dim=1))))
        xw41 = (Fun.relu(self.y1(xw41)))
        ...
        y1_word_patch = torch.empty((y1word.shape[0], y1word.shape[1]), dtype=torch.float32, device = 'cuda')  # number of batch and number of rows
        torch.empty_like(y1_word_patch)

        y2_word_patch = torch.empty((y2word.shape[0], y2word.shape[1]), dtype=torch.float32, device = 'cuda')
        torch.empty_like(y2_word_patch)

        y3_word_patch = torch.empty((y3word.shape[0], y3word.shape[1]), dtype=torch.float32, device = 'cuda')
        torch.empty_like(y3_word_patch)

        y4_word_patch = torch.empty((y4word.shape[0], y4word.shape[1]), dtype=torch.float32, device = 'cuda')
        torch.empty_like(y4_word_patch)

        for col_1 in range(y1word.shape[1]):  # looping for numbers of classes for this task
            y1_word_patch[:, [col_1]] = ((y1word[:, [col_1]] * xw11[:, [0]]) + (y1patch[:, [col_1]] * xw11[:, [1]])) / 2

        for col_2 in range(y2word.shape[1]):  # looping for numbers of classes for this task
            y2_word_patch[:, [col_2]] = ((y2word[:, [col_2]] * xw21[:, [0]]) + (y2patch[:, [col_2]] * xw21[:, [1]])) / 2

        for col_3 in range(y3word.shape[1]):  # looping for numbers of classes for this task
            y3_word_patch[:, [col_3]] = ((y3word[:, [col_3]] * xw31[:, [0]]) + (y3patch[:, [col_3]] * xw31[:, [1]])) / 2

        for col_4 in range(y4word.shape[1]):  # looping for numbers of classes for this task
            y4_word_patch[:, [col_4]] = ((y4word[:, [col_4]] * xw41[:, [0]]) + (y4patch[:, [col_4]] * xw41[:, [1]])) / 2
 
        return y1_word_patch.cuda(), y2_word_patch.cuda(), y3_word_patch.cuda(), y4_word_patch.cuda()


class CombineMultiOutputModelWeightedConcat2(nn.Module):
    def __init__(self, model_core, dd):
        super(CombineMultiOutputModelWeightedConcat2, self).__init__()
    ...
    def forward(self, x1_input, x2_input):
        ...
        # concat 16 and 16 nodes which makes it 32 and then it becomes 32 -> 16
        xw11 = self.bn5(Fun.relu(self.xw1(torch.cat((x11, x21), dim=1))))
        xw11 = Fun.softmax(Fun.relu(self.y1(xw11)))  # make it 16 -> 2

        xw21 = self.bn5(Fun.relu(self.xw1(torch.cat((x12, x22), dim=1))))
        xw21 = Fun.softmax(Fun.relu(self.y1(xw21)))

        xw31 = self.bn5(Fun.relu(self.xw1(torch.cat((x13, x23), dim=1))))
        xw31 = Fun.softmax(Fun.relu(self.y1(xw31)))

        xw41 = self.bn5(Fun.relu(self.xw1(torch.cat((x14, x24), dim=1))))
        xw41 = Fun.softmax(Fun.relu(self.y1(xw41)))
...
class CombineMultiOutputModelWeightedConcatAlexNet(nn.Module):
    def __init__(self, model_core, dd):
        super(CombineMultiOutputModelWeightedConcatAlexNet, self).__init__()

        self.resnet_model = model_core

        self.Conv_1 = ConvBNRelu1D(1, 64, 11, 4, 2)
        self.Conv_2 = ConvBNRelu1D(64, 192, 5, 1, 2)
        self.Conv_3 = ConvBNRelu1D(192, 384, 3, 1, 1)
        self.Conv_4 = ConvBNRelu1D(384, 256, 3, 1, 1)
        self.Conv_5 = ConvBNRelu1D(256, 256, 3, 1, 1)
        ...
        # network for dynamic weight
        self.xw1 = nn.Linear(384, 192)
        nn.init.xavier_normal_(self.xw1.weight)

        self.xw2 = nn.Linear(192, 96)
        nn.init.xavier_normal_(self.xw2.weight)

        self.xw3 = nn.Linear(96, 48)
        nn.init.xavier_normal_(self.xw3.weight)

        self.xw4 = nn.Linear(48, 24)
        nn.init.xavier_normal_(self.xw4.weight)

        self.y1 = nn.Linear(24, 2)  # this is to get 2 weight values only
        nn.init.xavier_normal_(self.y1.weight)

        self.d_out = nn.Dropout(0.5)

    def forward(self, x1_input, x2_input):
        x1_out = self.resnet_model(x1_input)  # 512 features
        x2_out = self.resnet_model(x2_input)  # 512 features

        n_batch_size = x1_out.shape[0]
        n_rows = x1_out.shape[1]

        x1_out_features = np.empty((n_batch_size, 1, n_rows))  # n_batch, nChannel, img_rows, img_cols
        x1_out_features = torch.from_numpy(x1_out_features).float()
        x1_out_features = torch.tensor(x1_out_features)

        for iBatch in range(0, n_batch_size):
            x1_out_features[iBatch, 0, :] = x1_out[iBatch, :]  # making 1st channel
        x1_out_features = x1_out_features.cuda()

        x2_out_features = np.empty((n_batch_size, 1, n_rows))  # n_batch, nChannel, img_rows, img_cols
        x2_out_features = torch.from_numpy(x2_out_features).float()
        x2_out_features = torch.tensor(x2_out_features)

        for iBatch in range(0, n_batch_size):
            x2_out_features[iBatch, 0, :] = x2_out[iBatch, :]  # making 1st channel
        x2_out_features = x2_out_features.cuda()

        x_conv_out_1 = self.Conv_1(x1_out_features.float())
        x_conv_out_1 = self.Conv_2(x_conv_out_1)
        x_conv_out_1 = self.Conv_3(x_conv_out_1)
        x_conv_out_1 = self.Conv_4(x_conv_out_1)
        x_conv_out_1 = self.Conv_5(x_conv_out_1)

        linear_features_1 = self.AvgPoolFlat(x_conv_out_1)
        linear_features_1 = self.Last_Linear_Layer(linear_features_1)

        x_conv_out_2 = self.Conv_1(x2_out_features.float())
        x_conv_out_2 = self.Conv_2(x_conv_out_2)
        x_conv_out_2 = self.Conv_3(x_conv_out_2)
        x_conv_out_2 = self.Conv_4(x_conv_out_2)
        x_conv_out_2 = self.Conv_5(x_conv_out_2)

        linear_features_2 = self.AvgPoolFlat(x_conv_out_2)
        linear_features_2 = self.Last_Linear_Layer(linear_features_2)

        # dynamic weight
        # concat 16 and 16 nodes which makes it 32 and then it becomes 32 -> 16
        xw11 = (Fun.relu(self.xw1(torch.cat((linear_features_1, linear_features_2), dim=1))))
        xw11 = (Fun.relu(self.xw2(xw11)))
        xw11 = (Fun.relu(self.xw3(xw11)))
        xw11 = (Fun.relu(self.xw4(xw11)))
        xw11 = (Fun.relu(self.y1(xw11)))  # make it 16 -> 2

        xw21 = (Fun.relu(self.xw1(torch.cat((linear_features_1, linear_features_2), dim=1))))
        xw21 = (Fun.relu(self.xw2(xw21)))
        xw21 = (Fun.relu(self.xw3(xw21)))
        xw21 = (Fun.relu(self.xw4(xw21)))
        xw21 = (Fun.relu(self.y1(xw21)))

        xw31 = (Fun.relu(self.xw1(torch.cat((linear_features_1, linear_features_2), dim=1))))
        xw31 = (Fun.relu(self.xw2(xw31)))
        xw31 = (Fun.relu(self.xw3(xw31)))
        xw31 = (Fun.relu(self.xw4(xw31)))
        xw31 = (Fun.relu(self.y1(xw31)))

        xw41 = (Fun.relu(self.xw1(torch.cat((linear_features_1, linear_features_2), dim=1))))
        xw41 = (Fun.relu(self.xw2(xw41)))
        xw41 = (Fun.relu(self.xw3(xw41)))
        xw41 = (Fun.relu(self.xw4(xw41)))
        xw41 = (Fun.relu(self.y1(xw41)))

        # heads
        y1word = Fun.softmax(self.y1o(linear_features_1), dim=1)
        y2word = Fun.softmax(self.y2o(linear_features_1), dim=1)
        y3word = Fun.softmax(self.y3o(linear_features_1), dim=1)
        y4word = Fun.softmax(self.y4o(linear_features_1), dim=1)  # should be sigmoid

        y1patch = Fun.softmax(self.y1o(linear_features_2), dim=1)
        y2patch = Fun.softmax(self.y2o(linear_features_2), dim=1)
        y3patch = Fun.softmax(self.y3o(linear_features_2), dim=1)
        y4patch = Fun.softmax(self.y4o(linear_features_2), dim=1)  # should be sigmoid
        
        y1_word_patch = torch.empty((y1word.shape[0], y1word.shape[1]), dtype=torch.float32, device = 'cuda')  # number of batch and number of rows
        torch.empty_like(y1_word_patch)

        y1_word_patch = torch.empty((y1word.shape[0], y1word.shape[1]), dtype=torch.float32, device = 'cuda')
        y2_word_patch = torch.empty((y2word.shape[0], y2word.shape[1]), dtype=torch.float32, device = 'cuda')
        y3_word_patch = torch.empty((y3word.shape[0], y3word.shape[1]), dtype=torch.float32, device = 'cuda')
        y4_word_patch = torch.empty((y4word.shape[0], y4word.shape[1]), dtype=torch.float32, device = 'cuda')
...
class CombineMultiOutputModelWeightedConcatAlexNet_1(nn.Module):
    def __init__(self, model_core, dd):
        super(CombineMultiOutputModelWeightedConcatAlexNet_1, self).__init__()
